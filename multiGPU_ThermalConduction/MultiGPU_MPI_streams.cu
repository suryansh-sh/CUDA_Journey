...
__global__ void ConductionGaussSidel(float *T, size_t _sizex, size_t _sizey) {
  ...
  if (tx > 0 && tx < _sizex - 1 && ty > 1 && ty < _sizey-2) { //boundary handled separately
    // clang-format off
...
__global__ void ConductionGaussSidel_b(float *T, size_t _sizex,
                                     size_t _loc_y) { //
  int tx = blockDim.x * blockIdx.x + threadIdx.x; // idx is taken
  if (tx > 0 && tx < _sizex - 1) {
    // clang-format off
        T[idx(_loc_y, tx, _sizex)] = 0.25f * (T[idx(_loc_y - 1, tx    , _sizex)] + 
                                              T[idx(_loc_y + 1, tx    , _sizex)] +
                                              T[idx(_loc_y    , tx - 1, _sizex)] + 
                                              T[idx(_loc_y    , tx + 1, _sizex)]);
    // clang-format on
  }
}
...
int main(int argc, char *argv[]) {
  ...
  cudaStream_t stream_b, stream_c; // boundary and compute streams
  cudaStreamCreateWithPriority(&stream_b, cudaStreamDefault, high_priority);
  cudaStreamCreateWithPriority(&stream_c, cudaStreamDefault, low_priority);
  ...
  dim3 blockDim_b(threadsPerBlockx, 1); //boundary is single row
  dim3 gridDim_b(blocksPerGridx, 1);

  auto start = MPI_Wtime(); // std::chrono::high_resolution_clock::now();
  for (int i = 0; i < iter; ++i) {
    // clang-format off
    ConductionGaussSidel_b<<<gridDim_b, blockDim_b, 0, stream_b>>>(d_T, mysize_x, 1           ); // compute boundary first and only synchronize them
    ConductionGaussSidel_b<<<gridDim_b, blockDim_b, 0, stream_b>>>(d_T, mysize_x, mysize_y - 2);
    ConductionGaussSidel  <<<gridDim  , blockDim  , 0, stream_c>>>(d_T, mysize_x, mysize_y    );
